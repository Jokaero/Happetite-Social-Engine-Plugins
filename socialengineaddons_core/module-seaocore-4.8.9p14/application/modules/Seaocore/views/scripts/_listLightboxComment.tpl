...
<?php $subjectModule = strtolower($this->subject()->getModuleName());?>
<?php $params['moduleName'] = $subjectModule;?>
<?php if(isset($this->subject()->listingtype_id)):?>
  <?php $subjectType .= '_' . $this->subject()->listingtype_id;?>
<?php endif;?>
<?php 
if(Engine_Api::_()->seaocore()->checkEnabledNestedComment($subjectType, $params)) : ?>
    <?php 
       include_once APPLICATION_PATH . '/application/modules/Nestedcomment/views/scripts/_nestedComment.tpl';
   ?>
 <?php else:?>
     <?php echo $this->action("list", "comment", "seaocore", array("type" =>$this->subject()->getType(), "id" => $this->subject()->getIdentity(), "show_bottom_post" => 1));?>
 <?php endif;?>
