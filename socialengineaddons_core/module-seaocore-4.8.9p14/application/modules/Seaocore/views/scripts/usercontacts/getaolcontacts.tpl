 ...
 * @version    $Id: getaolcontacts.tpl 2010-08-17 9:40:21Z SocialEngineAddOns $
...
if ($total > 0) { ?>
  <div class="header">	
			<div class="title">	
				<?php echo $this->translate("You have %s Aol contacts that you can add as your friends on",$total) . ' ' . Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.site.title') . "."; ?>
			</div>
			<div>
...
if ($total > 0) { ?>
		...
		<?php if  (empty($this->moduletype)) : ?>	
			 <div class="title">	
				  <?php echo $this->translate("You have ");?> <?php echo $total . $this->translate(" Aol contacts that are not members of ") .  Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.site.title') . "."; ?>
				</div>
  			<div>
...
if (!empty($this->addtononfriend)) { ?>
	...
	<div class="seaocore_user_contacts_buttons">
			 <?php if  (empty($this->moduletype)) : ?>		
			   <button name="invitefriends"  id="invitefriends" onclick="inviteFriends('aol');" style="float:left;margin-right:4px;"><?php echo $this->translate("Invite to Join");?></button>
			
			<?php else : ?>
			
			   <button name="invitefriends"  id="invitefriends" onclick="invitePreview('aol');" style="float:left;margin-right:4px;"><?php echo $this->translate("Invite to Join");?></button>	
			<?php endif ; ?>	
