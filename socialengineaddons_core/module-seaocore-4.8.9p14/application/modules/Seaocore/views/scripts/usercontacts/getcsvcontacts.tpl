 ...
 * @version    $Id: getcsvcontacts.tpl 2010-08-17 9:40:21Z SocialEngineAddOns $
...
if ($total > 0) { ?>
  <div class="header">	
			...
			<?php echo $this->translate("You have %s contacts that you can add as your friends on",$total) . ' ' . Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.site.title') . "."; ?>
			</div>
			<div>
...
if ($total > 0) { ?>
		<div class="header">	
			<?php if  (empty($this->moduletype)) : ?>	
			 <div class="title">	
				  <?php echo $this->translate("You have ");?> <?php echo $total . $this->translate(" contacts that are not members of ") .  Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.site.title') . "."; ?>
				</div>
  			<div>
...
<?php
  ...
  <input type="hidden" name="nonsitetotal_contacts"  id="nonsitetotal_contacts" value="<?php echo $total_contacts;?>"  >
		...
		<div class="seaocore_user_contacts_buttons">
			 <?php if  (empty($this->moduletype)) : ?>		
			   <button name="invitefriends"  id="invitefriends" onclick="inviteFriends('csv');" style="float:left;margin-right:4px;"><?php echo $this->translate("Invite to Join");?></button>
			
			<?php else : ?>
			
			   <button name="invitefriends"  id="invitefriends" onclick="invitePreview('csv');" style="float:left;margin-right:4px;"><?php echo $this->translate("Invite to Join");?></button>	
			<?php endif ; ?>	
