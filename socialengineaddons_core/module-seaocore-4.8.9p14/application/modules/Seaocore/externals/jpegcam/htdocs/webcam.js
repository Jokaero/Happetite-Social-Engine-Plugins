...
window.webcam = {
	...
	swf_url: 'application/modules/Seaocore/externals/jpegcam/htdocs/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: 'application/modules/Seaocore/externals/jpegcam/htdocs/shutter.mp3', // URI to shutter.mp3 sound
	...
	freeze: function() {
		if(document.getElementById("webcam_capture")) { document.getElementById("webcam_capture").style.display = 'none'; }
		if(document.getElementById("webcam_reset")) { document.getElementById("webcam_reset").style.display = 'inline'; }
		if(document.getElementById("webcam_save")) { document.getElementById("webcam_save").style.display = 'inline'; }
	...
	reset: function() {
		if(document.getElementById("webcam_capture")){ document.getElementById("webcam_capture").style.display = 'inline'; }
		if(document.getElementById("webcam_reset")){ document.getElementById("webcam_reset").style.display = 'none'; }
		if(document.getElementById("webcam_save")) { document.getElementById("webcam_save").style.display = 'none'; }
	...
	set_shutter_sound: function(enabled, url) {
		...
		this.shutter_url = url ? url : 'application/modules/Seaocore/externals/jpegcam/htdocs/shutter.mp3';
	},
		...
		switch (type) {
			...
			case 'error':
				...
				if (!this.fire_hook('onError', msg)) {
				    document.getElementById('temp_upload_image').innerHTML = '<div class="webcam_box_loading" style="height:480px;"><div class="tip"><span>No available cameras could be detected on your system. Please connect a camera and try again.</span></div></div><button type="button" onClick="parent.Smoothbox.close()" class="fright">Close</button>';
				}
				break;
