...
class Seaocore_Widget_SeaocoresIntegrationsController extends Engine_Content_Widget_Abstract {
    ...
    public function indexAction() {
        ...
        $this->view->isReview = $modName = Zend_Controller_Front::getInstance()->getRequest()->getModuleName();
        if ($modName == "sitestoreproduct")
            $modName = "sitestore";
        foreach ($rss as $getIntegratedModArray) {
            $tempModName = $getIntegratedModArray->integration_ptype();
            if (!empty($tempModName)) {
                $tempProductType = $getIntegratedModArray->ptype();
                if ($tempProductType == $modName) {
                    $integration_plugin_name = explode(",", $tempModName);
                    $integration_plugin_name = array_flip($integration_plugin_name);
                    break;
                }
            }
        }
        
        if( !empty($integration_plugin_name) )
            $integration_plugin_name = $this->getNodName($integration_plugin_name);

        // Loop over each channel item and store relevant data
        foreach ($rss as $item) {
            $product_type = $item->ptype();
            
            if( strstr($product_type, "sitereview") && strstr($item->link(), "socialengine-multiple-listing-types-plugin-listings") ){
...
// 		}
    ...
    protected function getNodName($modulesArray) {
        $getTempModulesArray = array();
        foreach ($modulesArray as $modName => $value) {
            switch ($modName) {
                case 'userconnection':
            ...
            if( strstr($module, "sitereviewlistingtype") ){
              $module = "sitereviewlistingtype";
            }
            
            $getTempModulesArray[$module] = $value;
        }
        return $getTempModulesArray;
    }
