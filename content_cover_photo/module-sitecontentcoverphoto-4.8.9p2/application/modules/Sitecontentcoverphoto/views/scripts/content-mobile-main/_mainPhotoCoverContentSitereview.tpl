		...
		<div class="ui-group-content">
				...
				<div class="siteuser_cover_profile_fields">
					<ul>
							<li>
								...
								<span> <?php echo $this->htmlLink('https://maps.google.com/?q='.urlencode($sitereview->location), $sitereview->location, array('target' => 'blank')) ?> </span>
							</li>
					</ul>
				</div>
			<?php endif;?>
			...
			<?php if(in_array('description', $this->showContent)):?>
				<?php if(Engine_Api::_()->getDbTable('otherinfo', 'sitereview')->getColumnValue($sitereview->getIdentity(), 'about')):?>
						<div><?php echo Engine_Api::_()->getDbTable('otherinfo', 'sitereview')->getColumnValue($sitereview->getIdentity(), 'about') ?></div>
				<?php elseif(strip_tags($sitereview->body)):?>
						<div><?php echo $this->viewMore(strip_tags($sitereview->body), 300, 5000) ?></div>
				<?php endif;?>
			<?php endif; ?>
		</div>
	<?php endif;?>	
